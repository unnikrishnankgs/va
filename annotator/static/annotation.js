...
class Annotation {
    ...
    constructor({fill, id, keyframes, type, user_info}) {
        // Fill of annotation
        this.fill = fill;
        ...
        // User Info like user_id, group_id, etc
    ...
    static newFromCreationRect(isImageSequence, creatingAnnotation) {
        var type = document.querySelector('input[name = "object"]:checked').value;
        var fill = Misc.getRandomColor(type);
        //window.alert(creatingAnnotation);
        return new Annotation({
            keyframes: [],
            fill: fill,
            id: fill,
            type: type,
            user_info: {"user_id" : ((creatingAnnotation == true) ? window.current_user : -1)},
        });
    }
